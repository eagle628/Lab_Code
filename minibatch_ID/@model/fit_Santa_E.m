function [theta, Jhistory] = fit_Santa_E(obj, t, u, y, theta, learning_ratio, rho, epsilon, burnin, beta_func, weight_b)
    ...
    mu_p = sqrt(learning_ratio)*randn(N_params, 1);
    ...
    for itr = 1:obj.max_iter
        ...
        if itr < burnin
            % exploration
            alpha = alpha + ( mu_p.^2 - learning_ratio./beta );
            mu_c = learning_ratio/beta.*(1 - g_p./g_c)./mu_p...
                 + sqrt(2*learning_ratio/beta*g_p).*randn(N_params,1);
        else
            % refinement
%             alpha = alpha;
            mu_c = 0;
        end
        mu_c = mu_c + (1 - alpha).*mu_p - learning_ratio*g_c.*dJ;
        theta = theta + g_c.*mu_c;
        % update
        mu_p = mu_c;
        g_p = g_c;
        if mod(itr, 10)==0
