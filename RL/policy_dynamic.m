classdef policy_dynamic < approximate_function_class
    ...
    methods
        function obj = policy_RBF(gen_ss_tridiag, sigma)
            obj.apx_function = gen_ss_tridiag;
            obj.sigma = sigma;
            obj.params = obj.apx_function.get_params();
            obj.internal_state  = zeros(obj.apx_fucntion.n, 1);
            obj.internal_state_ = zeros(obj.apx_fucntion.n, 1);
            obj.grad_internal_state  = zeros(obj.apx_function.n*length(obj.params));
            obj.grad_internal_state_ = zeros(obj.apx_function.n*length(obj.params));
        end
...
%             end
        ...
        function input = determistic_policy(obj, state)
        ...
        function grad = policy_grad_mu(obj, pre_input, state)
            [~,~,c,d] = obj.apx_function.get_ss(obj.params);
            [Abig, Bbig, Cbig, Dbig] = obj.get_sys_big(obj);
            ABCD = [blkdiag(Abig{:}), blkdiag(Bbig{:}); blkdiag(Cbig{:}). blkdiag(Dbig{:})];
            xy = ABCD*[obj.grad_internal_state; state];
