...
value = value_chainer_deep_net(py.chainer.optimizers.RMSprop(pyargs('lr',0.01)).setup(deep_net));
...
% K = lqr(model.A, model.B, eye(2), 1);
% sys_apx = apx_function.get_sys();
% controller.d = blkdiag(K, zeros(0, 3*belief_N-size(K, 2)));
% apx_function.set_sys(sys_apx);
apx_function.set_sys(controller);
% get_initial_guess(P, recorder, apx_function);

% policy = policy_dynamic_tf(apx_function, pi_sigma);
policy = policy_dynamic_ss(apx_function, pi_sigma);
policy_update_rule = TD_lambda(1e-6, 0);
...
%     train.train([0.4, 0], Te, train_seed,'mode-parallel', mode_parallel);
...
noise_seed = 1;
noise_power = 0;
x_rl = train.sim(test_ini, test_Te, [], noise_power, noise_seed,'mode-parallel', false);

figure
% plot(t, x_rl(:,2), 'r')
hold on

x_lqr = train.sim_lqrcontroller(test_ini,test_Te,noise_power,noise_seed);
plot(t,x_lqr(:,2),'b')

x_ori = train.sim_original(test_ini,test_Te,noise_power,noise_seed);
plot(t,x_ori(:,2),'c')

% net.add_controller(model.c_n, balred(model.sys_env, controller_n-2), train.Q_c, train.R)
net.add_controller(model.c_n, train.Q_c, train.R)
con_sys = net.get_sys_controlled(model.sys_all);
rng(noise_seed)
ddd = randn(length(t), 2)*noise_power;
